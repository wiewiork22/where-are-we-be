...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public EmployeeResponseDTO addEmployee(EmployeeRequestDTO request) {
		...
		var address = Address.builder().street(request.address().street()).city(request.address().city())
				.state(request.address().state()).postCode(request.address().postCode())
				.country(request.address().country()).lat(request.address().lat()).lng(request.address().lng()).build();

		addressRepository.save(address);
		var employee = Employee.builder().name(request.fullName()).email(request.email()).position(request.position())
	...
	public List<EmployeeResponseDTO> getAllEmployees() {
		return employeeRepository
				.getAll().stream().map(
						employee -> employeeMapper.mapToEmployeeResponseDTO(employee,
	...
	public void addAdminIfAbsent() {
		if (employeeRepository.findByEmail("admin@admin.com").isEmpty()) {
			...
			var address = Address.builder().street(Role.ADMIN.name()).city(Role.ADMIN.name()).state(Role.ADMIN.name())
					.postCode(Role.ADMIN.name()).country(Role.ADMIN.name()).lat(0.0).lng(0.0).build();
	...
	public void addEmployeeIfAbsent() {
		if (employeeRepository.findByEmail("employee@employee.com").isEmpty()) {
			...
			var address = Address.builder().street(Role.USER.name()).city(Role.USER.name()).state(Role.USER.name())
					.postCode(Role.USER.name()).country(Role.USER.name()).lat(0.0).lng(0.0).build();
